...
var (
	...
	ErrDuplicateUsername = errors.New("this username is taken")
)
...
func (s *UsersStore) getUserfromInvitation(ctx context.Context, tx *sql.Tx, token string) (*User, error) {
    ...
    JOIN user_invitations ui ON u.id = ui.user_id WHERE ui.token = $1 AND ui.expiry > $2`
...
func (s *UsersStore) GetByID(ctx context.Context, userID int64) (*User, error) {
	query := `SELECT users.id, username, email, password, created_at FROM users
